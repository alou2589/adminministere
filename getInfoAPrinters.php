<?php
    ...
    try {
        ...
        $sql='SELECT * FROM materiel m INNER JOIN type_materiel t ON(m.type_matos_id=t.id) INNER JOIN marque_matos mm ON(m.marque_matos_id=mm.id) 
                INNER JOIN fournisseur f ON(m.fournisseur_id=f.id) WHERE m.id=? AND t.nom_type_matos IN("Imprimante Noir et Blanc", "Imprimante Couleur", "Photocopieuse Multifonction")';
        $req=$db->prepare($sql);
